...
class AgentUserProviderTest extends TestCase
{
    ...
    public function testLoadUserByApiKeyWithDisabledAgent(): void
    {
        // Create mock objects
        $apiKey = $this->createMock(ApiKey::class);
        $agent = $this->createMock(Agent::class);
        ...
        $this->apiKeyRepository->method('findEnabledByKey')
            ->with('disabled-agent-key')
            ->willReturn($apiKey);
        
        // Expect exception
        $this->expectException(UserNotFoundException::class);
        $this->expectExceptionMessage('Agent is not enabled');
        
        // Call the method under test
        $this->agentUserProvider->loadUserByApiKey('disabled-agent-key');
    }
