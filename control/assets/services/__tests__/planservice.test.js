...
describe("planservice", () => {
    ...
    describe("Change Plan", () => {
        it("Should return response if successful", async () => {
            ...
            mock.onPost(`/app/billing/subscription/`+id+'/change/' + planName + '/' + paymentSchedule+'/'+currency).reply(200, {success: true});

            // when
            const result = await planservice.changePlan(id, planName, paymentSchedule, currency)

            // then
            expect(mock.history.post[0].url).toEqual(`/app/billing/subscription/`+id+'/change/' + planName + '/' + paymentSchedule+'/'+currency);
            expect(result.data).toEqual({success: true});
        });
        ...
        it("Should return error", async () => {
            ...
            mock.onPost(`/app/billing/subscription/`+id+'/change/' + planName + '/' + paymentSchedule+'/'+currency).reply(400, {success: false, error: "Error message here"});
    ...
    describe("Cancel", () => {
        it("Should return response if successful", async () => {
            ...
            mock.onPost(`/app/billing/subscription/`+subscriptionId+`/cancel`).reply(200, {success: true});

            // when
            const result = await planservice.cancel(subscriptionId);

            // then
            expect(mock.history.post[0].url).toEqual(`/app/billing/subscription/`+subscriptionId+`/cancel`);
            expect(result.data).toEqual({success: true});
        });
        ...
        it("Should return error", async () => {
            ...
            const subscriptionId = 'id';
            mock.onPost(`/app/billing/subscription/`+subscriptionId+`/cancel`).reply(400, {success: false, error: "Error message here"});
            try {
